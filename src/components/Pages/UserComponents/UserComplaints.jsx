  ...
  const handleSubmit = async (event) => {
    ...
    if ("geolocation" in navigator) {
      try {
        ...
        fetch(url)
          .then((res) => res.json())
          .then(async (data) => { // Wrap the .then() block in an async function
            ...
            const response = await fetch('http://localhost:8000/api/issues/report-issue', {
              method: 'POST',
              body: formData,
            });
            ...
            if (response.ok) {
              ...
              console.error('Failed to report issue:', response.status, response.statusText);
